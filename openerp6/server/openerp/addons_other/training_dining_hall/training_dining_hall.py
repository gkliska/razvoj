...
class res_partner_contact(osv.osv):
    ...
    _columns = {
        ...
        'grant_ids': fields.one2many('training.dining_hall.grant','contact_id', 'Grants'),
        #'group_id': fields.many2one('training.group', 'Group', help='The group that owns this contact.'),
        #'school_id': fields.related('group_id','school_id', type='many2one', readonly=True, size=30, store=True, relation='training.school', string='School'),
    }
...
class training_dining_hall_grant(osv.osv):
    _name = 'training.dining_hall.grant'
    _description = 'Defining grants conceded to the contact.'
    _rec_name = 'contact_id'
    _columns = {
        'contact_id': fields.many2one('res.partner.contact','Contact'),
        'date_from': fields.date('Date from', required = True, help="The start date of period."),
        'date_to': fields.date('Date to', required = True, help="The end date of period."),
        'monday': fields.boolean('Monday', help="Check it if the contact has been awarded the grant this day."),
        'tuesday': fields.boolean('Tuesday', help="Check it if the contact has been awarded the grant this day."),
        'wednesday': fields.boolean('Wednesday', help="Check it if the contact has been awarded the grant this day."),
        'thursday': fields.boolean('Thursday', help="Check it if the contact has been awarded the grant this day."),
        'friday': fields.boolean('Friday', help="Check it if the contact has been awarded the grant this day."),
        'saturday': fields.boolean('Saturday', help="Check it if the contact has been awarded the grant this day."),
        'sunday': fields.boolean('Sunday', help="Check it if the contact has been awarded the grant this day."),
        'type':fields.selection([
            ('daily','Daily'),
            ('monthly','Monthly'),
            ('monthly_exceded','Monthly (Exceded)'),
            ('fixed','Fixed Amount'),
        ],'Grant Type'),
        'grant_amount': fields.float('Grant Amount', digits_compute = dp.get_precision('Account')),
        'monthly_family_amount': fields.float('Monthly Amount by Family', digits_compute = dp.get_precision('Account'), help="The fixed monthly amount to pay by the family."),
    ...
    _defaults = {
        ...
        'saturday': lambda *a: False,
        'sunday': lambda *a: False,
        'type': lambda *a: 'daily',
        'valuation': lambda *a: 'percent',
    }
       
training_dining_hall_grant()
